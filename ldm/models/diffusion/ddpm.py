...
class LatentDiffusion(DDPM):
    ...
    def get_input(
        ...
        z_inpaint = self.get_first_stage_encoding(
            encoder_posterior_inpaint
        ).detach()
    ...
    def p_losses(
        ...
        if self.first_stage_key == "inpaint":
            # x_start=x_start[:,:4,:,:]
            noise = default(
                noise, lambda: torch.randn_like(x_start[:, :4, :, :])
            )
            x_noisy = self.q_sample(
                x_start=x_start[:, :4, :, :], t=t, noise=noise
            )
    ...
    def log_images(
        ...
        **kwargs,
        ...
        z, c, x, xrec, xc, mask, reference = self.get_input(
            batch,
