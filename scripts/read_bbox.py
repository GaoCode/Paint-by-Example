...
dataset_root = "/data/dataset/open-images"

os.mkdir(os.path.join(dataset_root, "bbox"))
dir_list = os.listdir(os.path.join(dataset_root, "images"))
dir_list.sort()
for dir_name in dir_list:
    print(dir_name)
    if "validation" in dir_name:
        csv_file_path = os.path.join(
            dataset_root, "annotations/validation-annotations-bbox.csv"
        )
    elif "test" in dir_name:
        csv_file_path = os.path.join(
            dataset_root, "annotations/test-annotations-bbox.csv"
        )
    else:
        csv_file_path = os.path.join(
            dataset_root, "annotations/oidv6-train-annotations-bbox.csv"
        )

    download_dir = os.path.join(dataset_root, "images", dir_name)
    label_dir = os.path.join(dataset_root, "bbox", dir_name)
    os.mkdir(label_dir)

    downloaded_images_list = [
        f.split(".")[0] for f in os.listdir(download_dir) if f.endswith(".jpg")
    ]
    ...
    for image in tqdm(images_label_list):
        try:
            ...
            for box in boxes:
                if not (
                    (box[1] - box[0]) * (box[3] - box[2]) > 0.8
                    or (box[1] - box[0]) * (box[3] - box[2]) < 0.02
                ):
